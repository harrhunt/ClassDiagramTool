...
int main(int, char**)
{
    ...
    Graph<ClassInfo> graph = GraphBuilder::build(".\\", recursive);
    ...
    while (!done)
    {
        ...
        ImGui::SetNextWindowSize(ImVec2(viewport->Size.x, viewport->Size.x - 140));
        ImGui::Begin("Class Diagram", NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBringToFrontOnFocus);
...
void drawGraph(Graph<ClassInfo> graph, ImDrawList* drawList) {
    std::unordered_map<int, ImVec2> nodeToPosMap; // Maps a specific class info to the location of the window for that class info
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    
    // nodes
    ImVec2 nodePos = ImVec2(viewport->Pos.x + 20, viewport->Pos.y + 100);
    for (Node<ClassInfo> node : graph.getNodes()) {
        nodeToPosMap.insert({node.getID(), nodePos});
        drawNode(node.getData(), nodePos, drawList);
    ...
    for (Edge<ClassInfo> edge : graph.getEdges()) {
        ...
        if (searchStart != nodeToPosMap.end() && searchEnd != nodeToPosMap.end()) {
            ImVec2 startPos = searchStart->second + ImVec2(55, 40);
            ImVec2 endPos = searchEnd->second + ImVec2(55, 40);
            drawList->AddBezierCurve(startPos, startPos + ImVec2(+50, 0), endPos + ImVec2(-50, 0), endPos, IM_COL32(100, 100, 100, 255), 3.0f);
        } 
    }

    // random line for testing
    int startID = graph.getNodes()[0].getID();
    int endID = graph.getNodes()[6].getID();
    auto searchStart = nodeToPosMap.find(startID);
    auto searchEnd = nodeToPosMap.find(endID);
    if (searchStart != nodeToPosMap.end() && searchEnd != nodeToPosMap.end()) {
        ImVec2 startPos = searchStart->second + ImVec2(55, 40);
...
void drawNode(ClassInfo classInfo, ImVec2 pos, ImDrawList* drawList) {
    ...
    if (ImGui::Button(&classInfo.getName()[0], ImVec2(95, 64)))
    ...
    // node box
    drawList->ChannelsSetCurrent(0);
    drawList->AddRectFilled(pos, pos + ImVec2(110, 80), IM_COL32(10, 10, 10, 255));
    drawList->AddRect(pos, pos + ImVec2(110, 80), IM_COL32(50, 50, 50, 255));
}
